...
export type PComplexFilterProps = {
  ...
  searchable?: boolean;
  currentFilters?: (string | menuItemType)[];
...
export function PComplexFilter({
  ...
  const handleSelectedInit = (event: React.MouseEvent<HTMLLIElement>, item: menuItemType) => {
    if (item.selected && setCurrentFilters) {
      setCurrentFilters((prev) => prev.filter((i) => i.text !== item.text));
      ...
      let value: menuItemType | undefined = undefined;
      let parent = _.clone(itemsHistory[itemsHistory.length - 2]);
      parent.forEach((child: menuItemType) => {
        child.children?.forEach((i: menuItemType) => {
          if (i === item) {
            value = child;
          }
        });
      ...
      if (value) {
        handleSelected(event, item.text, value);
      } else {
        console.log('error');
      }
    }
